...
pub async fn list_analytics(State(state): State<AppState>, Query(_q): Query<AnalyticsQuery>) -> Json<serde_json::Value> {
...
pub async fn post_trend(State(state): State<AppState>, Path(post_id): Path<String>) -> Json<serde_json::Value> {
