...
pub async fn create_account(State(state): State<AppState>, Json(input): Json<CreateAccount>) -> (StatusCode, Json<serde_json::Value>) {
    let id = Uuid::now_v7().to_string();
    match state.store.create_account(&id, &input).await {
        Ok(account) => (StatusCode::CREATED, Json(serde_json::json!({ "data": account }))),
        Err(e) => {
            let (status, body) = error_response(StatusCode::CONFLICT, "CREATE_FAILED", &e.to_string());
            (status, Json(serde_json::json!({ "error": body.error, "code": body.code })))
        }
    }
}

pub async fn update_account(State(state): State<AppState>, Path(id): Path<String>, Json(input): Json<UpdateAccount>) -> Json<serde_json::Value> {
...
pub async fn delete_account(State(state): State<AppState>, Path(id): Path<String>) -> Json<serde_json::Value> {
...
pub async fn refresh_token(State(_state): State<AppState>, Path(id): Path<String>) -> Json<serde_json::Value> {
